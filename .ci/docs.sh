...
function usage {
    ...
    echo "  check    Check that documentation will build properly"
...
if [[ "$COMMAND" == "install" ]]; then
    ...
    pip install sphinx nengo-sphinx-theme
    ...
    git push -fq "https://$GH_TOKEN@github.com/nengo/{{ PROJECT }}.git" gh-pages
else
    if [[ -z "$COMMAND" ]]; then
